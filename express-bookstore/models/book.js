...
class Book {
  ...
  static async create(data) {
    const isValid = validateBookCreate(data);
    if (!isValid) {
      throw { message: 'Validation Error', errors: validateBookCreate.errors };
    }
  ...
  static async update(isbn, data) {
    const isValid = validateBookUpdate(data);
    if (!isValid) {
      throw { message: 'Validation Error', errors: validateBookUpdate.errors };
    }
    const result = await db.query(
      `UPDATE books SET 
            amazon_url=($1),
